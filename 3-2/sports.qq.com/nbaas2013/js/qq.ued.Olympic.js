(function($,$$){
	//焦点图
	$.OlyFocus = function(args){
		return new _OlyFocus(args);
	},_OlyFocus = function(args){
	...
	_OlyFocus.prototype = {
		init:function(){
			...
			this.left.bind("mouseover",function(){
				...
				_self.slid.go({marginLeft:"-"+px*_self.index+"px"},'slow','backIn',function(){
					_self.change();
				});	
			})
			...
			this.nubmer.each(function(o,i){
				o.click(function(){
					_self.slid.go({marginLeft:"-"+px*i+"px"},'slow','backIn',function(){
						_self.index = i;
						_self.change();
					});
				});
		...
		change:function(){
			...
			this.nubmer.each(function(o,i){
				o.node.className = "";
				_self.imgs.item(i).parent(1).node.className = "";
				if(_self.txt){
			...
			this.imgs.item(_self.index).parent(1).node.className = "on";
		},
		setAuto:function(){
...
})(K,KK)/*  |xGv00|8c7a7a96dafaf83212279f5931a7fe0d */