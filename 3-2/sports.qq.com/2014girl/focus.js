...
_tab.prototype = {
    int: function() {
        var _this = this;
        var current = 0;
        jQuery("#focus01 .scrollUl").css({width:this.iWidth*this.len});
...
yundong: function(idx) {
	...
	for (var i = 0, j = this.cardBtnLi.length; i < j; i++) {
			  ...
			  _this.cardBodyDiv[i+1].className = "";
			  _this.cardBodyDiv[i+2].className = "";
			  _this.txtLi.eq(i).fadeOut();
     }
			 _this.cardBtnLi[idx].className = "current";
			 _this.cardBodyDiv[idx+1].className = "current";
...
_tab2.prototype = {
    int: function() {
        var _this = this;
        var current = 0;
        jQuery("#focus02 .scrollUl").css({width:this.iWidth*this.len});
		  this.cardBtnLi.each(function(index,obj){
		     jQuery(obj).bind("click",function(){
			  _this.iNow = index;
			  _this.yundong(_this.iNow);
...
/*  |xGv00|a248762f76e747759d228dbaa22caa1c */