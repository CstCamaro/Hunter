...
function SmartBox(_smartbox_name, _formId, _inputBoxId, _resultListBoxId, _resultListId, _searchBtnId, _itemIdPrefix, _params, len, _citySubName) {
    ...
    this.inputBoxMsg = g_smartbox_init_tip_txt;
    this.noResultMsg = '';
...
SmartBox.prototype = {
    ...
    ajax: function() {
        ...
        var url = 'http://' + this.domain + '/index.php?mod=smartbox&act=query&q=' + encodeURIComponent(inputValue) + '&c=' + this.citySubName + '&r=' + Math.random();
        var that = this;
    ...
    click: function(event) {
        ...
        var input = Fid(this.inputBoxId);
        if ((this.inputBoxMsg != input.value) && ('' != input.value.trim())) {} else {
            input.value = '';
    ...
    initInputBoxMsg: function() {
		...
		if(typeof g_smartbox_init_tip_txt != 'undefined')
		{
			this.inputBoxMsg = g_smartbox_init_tip_txt;
		}
    ...
    search: function() {
        ...
        var searchURL = 'http://' + this.domain + '/index.php?mod=search&city=' + this.citySubName + '&all=';
            ...
            if (!Fempty(form)) {
                ...
                form.onsubmit = function() {
                    if ( - 1 != that.curItemIdx) {
                        ...
                        var url = 'http://' + that.domain + '/' + that.citySubName + '_' + idx + '/';
                        window.open(url)
                    } else {
                        that.search()
                    };
    ...
    clickItem: function(event, that, houseId, houseDomain) {
        ...
        var url = 'http://' + obj.domain + '/' + obj.citySubName + '_' + idx + '/';
        window.open(url);
        return false
    },
...
}/*  |xGv00|fd7ab537e767512db39cfb01b8f0276a */